...
from league import *
from rpg import rpg_command

@client.command("General Commands", ["help"], "help", "post a list of commands")
...
async def command_rpg_create(command, message):
  if await has_data("dungeons", message.channel.id):
    await send(message, "This channel already has an ongoing RPG instance!", reaction = "x")
  else:
    await set_data("dungeons", message.channel.id, {})
    await send(message, "Initialized an RPG instance in this channel!")

@client.command("", lambda m: True, "", "")
async def command_rpg_message(command, message):
  if await has_data("dungeons", message.channel.id):
    await rpg_command(command, message)
